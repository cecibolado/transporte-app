...
router.get("/", async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, error: "Error al obtener conductores" });
    }
});
...
router.post(
    ...
    async (req, res) => {
        ...
        try {
            ...
            res.status(500).json({ success: false, error: "No se pudo crear el conductor" });
        }
    }
);
...
router.put(
    ...
    async (req, res) => {
        ...
        try {
            ...
            res.json({ success: true, mensaje: "Conductor actualizado correctamente" });
        } catch (err) {
            console.error("Error al actualizar conductor:", err);
            res.status(500).json({ success: false, error: "No se pudo actualizar el conductor" });
        }
    }
);
...
router.delete("/:id", async (req, res) => {
    ...
    try {
        ...
        res.json({ success: true, mensaje: "Conductor eliminado correctamente" });
    } catch (err) {
        console.error("Error al eliminar conductor:", err);
        res.status(500).json({ success: false, error: "No se pudo borrar el conductor" });
    }
});
