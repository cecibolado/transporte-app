...
router.get(
  ...
  async (req, res) => {
    ...
    const [rows] = await db.execute(
      ...
      "SELECT id, username, apellido, nombre, rol FROM usuarios WHERE id=?",
      [id]
    );
...
router.post(
  "/",
  //verificarAutenticacion, // Esta linea te dice que tenes que estar logeada
  //verificarAutorizacion("admin"), // Y esta te dice que tenes que tener rol de admin, por eso si no cumplis estas 2 cositas no vas a poder crear el usuario. Entonces, lo que podes hacer es sacar estos middleware para que quede abierto el registro para que puedas crear usuario sin la necesidad de estar login
